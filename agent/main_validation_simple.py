...
from src.requirement_refiner import RequirementRefiner
from src.simple_validator import Validator
from src.utils import construct_dir,from_req_to_json
import json
import os
import argparse
import logging
from src.utils import extract_class_sequence_diagram
import datetime
...
def main():
    ...
    parser.add_argument("--model", default="deepseek-v3.2", help="Model type (e.g., gemini-2.5-flash, gpt-4)")
    parser.add_argument("--method", default="simple_agent", help="Method name (e.g., our_method, baseline)")
    ...
    parser.add_argument("--uc-case-dir", default="datasets/eTour/req", help="The directory containing use case files")
    parser.add_argument("--start-uc-id", type=int, default=23, help="Start UC id")
    parser.add_argument("--end-uc-id", type=int, default=25, help="End UC id")
    ...
    for uc_id in range(args.start_uc_id,args.end_uc_id+1):
        ...
        args.use_case_dir= "data/eTour/eTour/req"
        try:
        ...
        for i in range(1,args.check_rounds+1):
            ...
            validation_result = validator.validate_design(
                requirements=use_case_json,
                design_model=design_model
            )
            tracibility_info = {
                "class_diagram": validation_result["class_diagram"],
                "sequence_diagram": validation_result["sequence_diagram"],
                "summary": validation_result.get("summary", ""),
                "repair_suggestions": validation_result.get("repair_suggestions", [])
            }
            trace_class = validation_result["class_diagram"]
            trace_sequence = validation_result["sequence_diagram"]
            class_json_graph = {"mode": "no_graph_llm_validator", "issues": trace_class}
            sequence_json_graph = {"mode": "no_graph_llm_validator", "issues": trace_sequence}
            ...
            report = validator.format_report(tracibility_info, mode="Design")
        ...
        for i in range(1,args.check_rounds-1):
            ...
            feedback = validator.validate_code(
                design_model=final_design,
                implementation_code=implementation_code
            )
            code_tracibility = {
                "class_code_traceability": feedback["class_diagram"],
                "sequence_code_traceability": feedback["sequence_diagram"],
                "summary": feedback.get("summary", ""),
                "repair_suggestions": feedback.get("repair_suggestions", [])
            }
            ...
            implementation_code = developer.update_implementation(feedback=feedback)
            developer.memory['updated_implementation'] = implementation_code
